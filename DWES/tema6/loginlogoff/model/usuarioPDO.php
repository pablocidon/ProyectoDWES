...
class UsuarioPDO{
    ...
    public static function validarUsuario($codUsuario, $password) {
        ...
        $resConsulta= BDPDO::ejecutarConsulta($consulta,[$codUsuario,$password]); //Ejecutamos la consulta
    ...
    public static function registrarUsuario($codUsuario, $password, $descripcion) {
        $registrado=false;
        $consulta="INSERT INTO Usuarios (CodUsuario,Password,DescUsuario,Perfil)VALUES (?,?,?,'Usuario')";
        $resConsulta= Bdpdo::ejecutarConsulta($consulta,[$codUsuario,$password,$descripcion]);
    ...
    public static function comprobarExisteUsuario($codUsuario){
        ...
        $resConsulta= Bdpdo::ejecutarConsulta($consulta, [$codUsuario]);
        if ($resConsulta->rowCount()==1){
            $registrado=true;
        }
        return $registrado;
    ...
    **/
    public static function editarUsuario($descripcion,$password,$codUsuario){
        ...
        $resConsulta= Bdpdo::ejecutarConsulta($consulta,[$descripcion,$password,$codUsuario]);
        if ($resConsulta->rowCount()==1){
            $editado=true;
        }
        return $editado;
